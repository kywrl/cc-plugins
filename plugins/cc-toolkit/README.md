...
**「一轮」的定义**：从**你按下回车**到这一次回复结束。中间模型可能调用几十上百次
API（每次之间还夹着工具执行），这些都属于同一轮 —— 三个读数都是在这一整轮上算出来的，
不是某一次调用的。
...
- 纯解码速度按**每一次 API 调用内部**的块间隔分别测量再求和（一轮里调用之间夹着的
  工具执行时间不算进去 —— 算进去会把读数压低近十倍）。累加跨度不足 300ms 时
  没有可测区间，那一格显示 `—`，不报假数字；
- `≈` 只标在**具体用了估算的那个数**前面。一轮里 usage 只落在少数 API 调用上
  （该 provider 的常态），所以经常会出现「首字是精确值、每秒输出标 ≈」这种组合 ——
  这不是 bug，是数据源只给了部分精确值。

## 统计视图与每轮读数的口径差异

这两者的「一轮」不是同一个东西，是**有意**的：

| | 一轮 = | 用在哪 |
|---|---|---|
| 每轮读数（Stop hook） | 你发一条消息 → 回复结束 | 回答「刚才这次等了多久、多快」 |
| 统计视图（`/cc-toolkit:tps`） | 一次 API 调用 | 回答「纯解码速度的分布长什么样」 |

统计视图按单次调用切分，是因为那个粒度能自然地剔掉工具执行时间，
