...
"use strict";
/**
 * tps-statusline — 把 tok/s 塞进 Claude Code 的状态栏
 *
 * 状态栏每次刷新都会调用这个脚本，所以它必须便宜：
 *   1. 先读 ~/.tmp/tps-watch-<session>.json 缓存（由 Stop hook 写）；
 *   2. 缓存缺失或过期 → 只回放 transcript 末尾 400KB 重算一次，然后写回缓存。
 * 绝不做全量回放。
 *
 * 用法（放进 settings.json 的 statusLine.command）:
 *   node "${CLAUDE_PLUGIN_ROOT}/scripts/tps-statusline.js"
 *
 * 环境变量:
 *   TPS_WATCH_STATUSLINE_PREFIX   前缀，默认 "⚡ "
 *   TPS_WATCH_MIN_TOKENS=30       低于该 token 数不显示
 *   TPS_WATCH_STATUSLINE_CACHE_MS 缓存有效期，默认 45000
 *   TPS_WATCH_DISABLE=1           禁用（输出空）
 */
...
if (env.TPS_WATCH_DISABLE === "1") process.exit(0);
...
function main() {
  ...
  const cacheMs = parseInt(env.TPS_WATCH_STATUSLINE_CACHE_MS || "45000", 10) || 45000;
  const prefix = env.TPS_WATCH_STATUSLINE_PREFIX != null ? env.TPS_WATCH_STATUSLINE_PREFIX : "⚡ ";
