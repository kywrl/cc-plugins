...
**状态栏的「首字」偶尔也会整段不显示**：状态栏为了便宜只回放会话末尾 400KB，
切点可能正好落在某一轮的用户行与其首个内容块之间 —— 那一轮的起点锚就没读到。
这时「首字」和「整轮耗时」都少了一整段 prefill，插件按「测不出来」处理：不显示首字，
也不让这一轮进入中位数/p90（它的速度会系统性偏高）。这种情况只影响紧挨切点的那一轮，
换一轮刷新就恢复正常；实在在意就把 `CC_TOOLKIT_STATUSLINE_CACHE_MS` 调小，
让缓存更多由 Stop hook（回放 2MB）来写。
