...
| `每秒输出`（decode） | 各次 API 调用内部（首块 → 末块）的跨度之和 | 中位 **227–486 tok/s** |
| `整轮`（tps，默认不显示） | 整轮 token ÷ 整轮耗时（含 prefill 与工具执行） | 中位 32–71 |

> `每秒输出` 按**每一次 API 调用**分别测跨度再求和，不含调用之间夹着的工具执行时间。
...
**`每秒输出` 显示 `—` 是正常的**：纯解码速度要按**每一次 API 调用内部**的块间隔测出来
再求和（调用之间夹着的工具执行时间不算进去）。累加跨度不足 300ms 就没有可测区间 ——
...
- **「一轮」= 你发一条消息 → 回复结束**（每轮读数用这个口径）。中间模型可能调用几十上百次
  API，这些都属于同一轮，三个读数是在整轮上算出来的。统计视图（`/cc-toolkit:tps`）则按
  **一次 API 调用**切分 —— 那个粒度能自然地剔掉工具执行时间，让历史样本之间可比。
  两者口径不同是有意的。
- **耗时起点**：**你按下回车**那一刻（不是第一个内容块落盘的时刻，也不是上一次 API 调用结束）。
  否则首字延迟（TTFT）会被漏掉，速度看起来偏快。工具结果回流与技能注入（`isMeta`）不算新一轮。
- **`≈` 的粒度**：只标在具体用了字符估算的那个数前面。一轮里 usage 只落在少数 API 调用上
