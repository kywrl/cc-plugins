...
装好即在每轮回复后看到本轮步数 / 每秒输出 / 缓存命中；`/cc-toolkit:tps` 查看历史分布、趋势、离群样本与分层归因。
...
当前（最近一轮，已结束）: 44 tok/s  141 tok / 3.2s
  本轮 12 步
  拆分: 解码 88 tok/s (跨 1.3s)
  缓存命中 92%
最近 5 条已完成的响应:
  01:44:42    696 tok /   4.6s =  151 tok/s 解码 210
  01:44:46    332 tok /   3.4s =   99 tok/s 解码 165
  01:44:54   1657 tok /   7.8s =  212 tok/s 解码 331
  01:45:01   1331 tok /   7.2s =  184 tok/s 解码 288
  01:45:05    349 tok /   3.9s =   89 tok/s 解码 142
中位 151 tok/s | p90 212 | 最快 212 (01:44:54) | 最慢 89 (01:45:05)
纯解码口径（5/5 条可拆分）: 中位 227 tok/s
```

> 样本列表按**步**切分（见下节），所以每一行都恰好是一步，不再单列步数。
> 「本轮 N 步」只出现在当前轮那一行 —— 那才是轮级读数。
...
| 命令 | 作用 |
| --- | --- |
| `/cc-toolkit:tps [条数]` | 多行快照：当前一轮（含**本轮步数**）+ **纯解码拆分** + 最近 N 条（按步）+ 中位/p90/最快/最慢 + 趋势与离群样本，并附**分层归因**（按 effort / 模型 / 技能 / MCP / 是否带 thinking 分组对比）与会话级事实（缓存命中、API 重试、截断）。参数默认 10，可加 `--all` 回放整个会话文件 |
...
| `CC_TOOLKIT_SHOW` | `steps,decode,cache` | 每轮那行包含哪些字段，用 ` \| ` 连接。默认就是三个原始读数。可选 `steps`（本轮步数）`decode`（每秒输出/纯解码）`cache`（缓存命中）`ttft`（首块等待，见上文为何默认关闭）`tps`（整轮速度）`median`（近 9 条中位）`tokens`（本轮 token）`thinking` `model` `effort` `skill`。例如只留速度：`CC_TOOLKIT_SHOW=decode,tps` |
...
**`ttft`（首块等待）为什么不在默认三格里**：它测的是「你按下回车 → 本轮第一个内容块落盘」，
含**首块自身的生成时间**。而首块大小在一轮之间差几十倍 —— 实测首块 ≥1000 字符时中位 26–29s，
<50 字符时 9–13s。也就是说这个数会被「首块更大」推高，跨轮次比较时容易误读成「等得更久」。

更根本的是，模型真的吐出第一个 token 的那一刻，在块级落盘的日志里**不可观测** ——
日志只在整块写完时才写一行。所以这个位置上测得到的只有「首块落盘」，名字也叫「首块等待」。
它不再是默认读数，想看得显式加进 `CC_TOOLKIT_SHOW`。
